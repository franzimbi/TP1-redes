...
class ProtocolServer:
    ...
    def recv_file(self, path):
        ...
        self.logger.log("[server_protocol]: recibiendo archivo {name}", 0)
        # recibo el tamanio del archivo
        data_bytes = self.socket.recv(32)
        size_file = int.from_bytes(data_bytes, byteorder='big')
        self.logger.log("[server_protocol]: archivo {name} tiene tamanio {size_file}", 1)
        # recibo el archivo
        ...
        if not os.path.exists(path):
            ...
            return
        self.logger.log("[server_protocol]: el path al archivo {name}: {size_file} existe", 1)
        with open(path_complete, 'wb') as f:
            self.logger.log("[server_protocol]: creando el archivo {path_complete} y recibiendo", 1)
            for _ in range(int(size_file)):
                ...
                f.write(data)
        f.close()
        self.logger.log("[server_protocol]: archivo {name} recibido correctamente", 1)
