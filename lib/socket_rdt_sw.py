...
from lib.logger import HIGH_VERBOSITY, LOW_VERBOSITY
...
class SocketRDT_SW:
    ...
    def _update_rtt(self, sample_rtt):
        alpha = 0.125
        beta = 0.25
        self.estimated_rtt = (1 - alpha) * self.estimated_rtt + alpha * sample_rtt  # noqa: E501
        self.dev_rtt = (1 - beta) * self.dev_rtt + beta * abs(sample_rtt - self.estimated_rtt)  # noqa: E501
        self.timeout_interval = self.estimated_rtt + 4 * self.dev_rtt
    # Para Servidor
    ...
    def accept(self):
        self.logger.log("[SW-accept] aceptando con 3 way handshake", LOW_VERBOSITY)  # noqa: E501
        ...
        if packet.want_SYN():
            if len(self.sockets) >= self.socket_max:
                self.logger.log(
                    "[SW-accept] No se pueden aceptar más conexiones por limite", LOW_VERBOSITY)  # noqa: E501
    ...
    def connect(self, adress):
        ...
        while not server_syn_ack.want_SYN() and counter < TOTAL_RETRIES:
            self.logger.log("[sw-connect] Enviando SYN, esperando SYN-ACK", HIGH_VERBOSITY)  # noqa: E501
        ...
        if server_syn_ack.want_SYN():
            ...
            self.logger.log(
                f"[sw-connect] Conexión establecida con {self.socket.getsockname()}", LOW_VERBOSITY)  # noqa: E501
